...
@property(nonatomic,strong) LCRegisteredViewModel *viewModel;
@end
...
- (void)viewDidLoad {
    ...
    NSString *phoneNumplaceholder= @"请输入手机号";
    ...
    NSString *passwordplaceholder= @"密码: 6-18位,区分大小写";
    ...
    NSString *renicknameplaceholder= @"请再次输入密码";
    ...
    //middle bottom line
    UIView *middleBottomlineView = [UIView new];
    middleBottomlineView.backgroundColor = [KDColor getX0Color];
    [self.view addSubview:middleBottomlineView];
    [middleBottomlineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.mas_equalTo(0);
    ...
    NSMutableAttributedString *nicknameplaceholderAr = [[NSMutableAttributedString alloc]initWithString:nicknameplaceholder];
    [nicknameplaceholderAr addAttribute:NSFontAttributeName value:[[KDFont new]getF30Font] range:NSMakeRange(0, nicknameplaceholderAr.length)];
    [nicknameplaceholderAr addAttribute:NSForegroundColorAttributeName value:[KDColor getX0Color] range:NSMakeRange(0, nicknameplaceholderAr.length)];
    self.nicknameTextField.attributedPlaceholder = nicknameplaceholderAr;
    [_nicknameTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(middleBottomlineView.mas_bottom).mas_equalTo(-9);
        make.left.mas_equalTo(30);
        make.width.mas_equalTo(200);
        make.height.mas_equalTo(25);
    }];
    ...
    [bottomLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.mas_equalTo(0);
        make.top.mas_equalTo(middleBottomlineView.mas_bottom).mas_equalTo(44);
        make.height.mas_equalTo(0.5);
    }];
    ...
    [nextStepBT addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
        @strongify(self)
        MYLog(@"%@下一步",self);
        !self.viewModel.registAccount ? : self.viewModel.registAccount(self.phoneNumTextField.text,self.passwordTextField.text,self.repasswordTextField.text,self.nicknameTextField.text,self.validationTextField.text);
    }];
}
