...
- (UIViewController *)initializeViewControllerAtIndex:(NSInteger)index andViewModel:(BaseViewModel *)viewModel {
    if ([self.dataSource respondsToSelector:@selector(pageController:viewControllerAtIndex:)]) {
        return [self.dataSource pageController:self viewControllerAtIndex:index];
    }
    return [[self.viewControllerClasses[index] alloc] initWithViewModel:viewModel];
}
...
- (void)wm_addViewControllerAtIndex:(int)index {
    ...
    if (self.values.count == self.childControllersCount && self.keys.count == self.childControllersCount) {
        viewController = [self initializeViewControllerAtIndex:index andViewModel:self.values[index]];
        [viewController setValue:self.values[index] forKey:self.keys[index]];
    }else{
        viewController = [self initializeViewControllerAtIndex:index];
    }
    [self addChildViewController:viewController];
...
- (BOOL)wm_isInScreen:(CGRect)frame {
    ...
    CGFloat WScreenWidth = self.scrollView.frame.size.width;
    
    CGFloat contentOffsetX = self.scrollView.contentOffset.x;
    if (CGRectGetMaxX(frame) > contentOffsetX && x-contentOffsetX < WScreenWidth) {
        return YES;
    } else {
        return NO;
    }
